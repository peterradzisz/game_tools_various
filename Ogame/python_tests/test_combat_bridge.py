...
def test_batch_runs_in_multiprocessing() -> None:
    ...
    # 4 different seeds must produce 4 different win_probability values
    # (a flake-free smoke check; if seeds aren't threaded through correctly
    # the four results would all be identical).
    win_probs = [r["win_probability"] for r in results]
    assert len(set(win_probs)) > 1, (
        f"expected distinct win_probability for distinct seeds, got {win_probs}"
    )
