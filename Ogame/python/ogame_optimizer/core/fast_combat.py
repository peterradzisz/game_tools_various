...
SHIP_STATS: Dict[str, dict] = {
    ...
    "pathfinder":       {"atk": 300,    "shield": 100,   "hull": 20000},
    "recycler":         {"atk": 1,      "shield": 10,    "hull": 9600},
}

DEFENSE_STATS: Dict[str, dict] = {
    "rocket_launcher":   {"atk": 80,   "shield": 20,    "hull": 400},
    "light_laser":       {"atk": 100,  "shield": 25,    "hull": 800},
    "heavy_laser":       {"atk": 250,  "shield": 100,   "hull": 1500},
    "gauss_cannon":      {"atk": 1100, "shield": 200,   "hull": 6000},
    ...
    "large_shield_dome": {"atk": 0,    "shield": 10000, "hull": 2000},
}
    ...
    for k, v in fleet.items():
        if v > 0 and k in SHIP_STATS:
            ...
            side[k] = {
                ...
                "hull": s["hull"] * v * hull_mult * 0.1,
                "base_shield": s["shield"] * shield_mult,
                "unit_hull": s["hull"] * hull_mult * 0.1,
                "atk": s["atk"] * atk_mult,
            }
...
def _fire(attacker_side: dict, defender_side: dict, rng: random.Random):
    ...
    for k_atk, atk in attacker_side.items():
        ...
        for k_def, d in defender_side.items():
            ...
            # Shield bounce: individual shot can't pierce unit shield
            if per_shot < d["base_shield"]:
                shield_dmg = per_shot * shots_at_type
                sigma = math.sqrt(max(shield_dmg, 1))
                actual_shield_dmg = max(0.0, shield_dmg + rng.gauss(0, sigma * 0.1))
                d["shields"] = max(0, d["shields"] - actual_shield_dmg)
                continue
            ...
            if hull_dmg > 0:
                ...
                if d["hull"] <= 0:
                    ...
                    if max_hull > 0:
                        survival = max(0.0, d["hull"] / max_hull)
